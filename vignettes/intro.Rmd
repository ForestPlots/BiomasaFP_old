...
- Geographic coordinates, dates of the first and last census, and number of censuses (`SummaryPlotviews`).
...
The allometric equations called by `SummaryAGB`,`AGBch` and `MeanWtAGB` can also be accessed individually, and return estimates of AGB and, where height is used in the equatio, estimated height for each tree in each census. Again, these use DBH4 by default, but this can be changed by the user.
