...
SummaryPlotviews <- function (xdataset){
        ...
        xdu <- xdataset[,c('PlotID','PlotCode','Country','PlotViewID','Census.No','Census.Mean.Date','LatitudeDecimal', 'LongitudeDecimal')]
        udfile <- unique(xdu)
        udfile
        # minimum
        ...
        # maximum
        maxx <- aggregate(cbind(Census.Mean.Date, Census.No) ~ PlotViewID +Country + PlotCode +
                                   LatitudeDecimal + LongitudeDecimal, data = xdu, FUN=max)
        colnames(maxx) <- c('PlotViewID', 'Country','PlotCode','LatitudeDecimal','LongitudeDecimal','Last.Census.Date','No.Censuses')
        #merge
        PlotsSumm <-merge(minx, maxx, by = 'PlotViewID', x.all = TRUE)
        PlotSumma<- PlotsSumm[,c('Country','PlotCode', 'PlotViewID', 'LatitudeDecimal','LongitudeDecimal','First.Census.Date','Last.Census.Date','No.Censuses')]
        PlotSummb <-PlotSumma[order(PlotSumma$Country, PlotSumma$PlotCode, PlotSumma$PlotViewID, decreasing=FALSE), ]
        PlotSummb
